...
let weatherOneStatusHumidity =document.querySelector(".weather-one-status-humidity")
...
let weatherThreeTemprature = document.querySelector(".weather-three-temprature")
let weatherTwoBTemprature =document.querySelector(".weather-twoB-temprature")
let weatherThreeBTemprature =document.querySelector(".weather-threeB-temprature")
...
async function requesttwo( searchvalue = 'cairo' ) {
    let req = await fetch(
      `http://api.weatherapi.com/v1/forecast.json?key=b5abd05e0afe42b2af402838241412&days=3&q=${searchvalue}`
    );
    ...
    weatherTwoBigicon.setAttribute("src" , `https:${data.forecast.forecastday[1].day.condition.icon}`)
    weatherThreeBigicon.setAttribute("src" , `https:${data.forecast.forecastday[2].day.condition.icon}`)
    weatherTwoTemprature.innerHTML = data.forecast.forecastday[1].day.maxtemp_c+ "&#8451" 
    weatherThreeTemprature.innerHTML =data.forecast.forecastday[2].day.maxtemp_c+ "&#8451" 

    weatherTwoBTemprature.innerHTML=  data.forecast.forecastday[1].day.mintemp_c+ "&#8451" 
    weatherThreeBTemprature.innerHTML=  data.forecast.forecastday[2].day.mintemp_c+ "&#8451" 

    weatherTwoStatus.innerHTML =data.forecast.forecastday[1].day.condition.text
    weatherThreeStatus.innerHTML =data.forecast.forecastday[2].day.condition.text
